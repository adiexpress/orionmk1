...
if is_pi:
    ...
    _output_info = _hef.get_output_vstream_infos()[0]
...
def _detect_hailo(frame):
    ...
    #opencv reads BGR but hailo needs rgb so convert
    resized = cv2.cvtColor(cv2.resize(frame, (_input_w, _input_h)), cv2.COLOR_BGR2RGB)
...
def _detect_cpu(frame):
    ...
    results = _cpu_model(frame, verbose = False)
